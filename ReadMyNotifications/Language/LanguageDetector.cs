...
namespace ReadMyNotifications.Language
{
    public class LanguageDetector
    {
        private static readonly Regex UrlRegex = new Regex("https?://[-_.?&~;+=/#0-9A-Za-z]{1,2076}");
        private static readonly Regex EmailRegex = new Regex("[-_.0-9A-Za-z]{1,64}@[-_0-9A-Za-z]{1,255}[-_.0-9A-Za-z]{1,255}");
        ...
        public async Task AddLanguages(params string[] languages)
        {
            ...
            {
                var storageFile = await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///Language/Profiles/" + language + ".bin.gz"));
                using (var langFile = await storageFile.OpenReadAsync())
                    using (var langStream = langFile.AsStreamForRead())
                        using (Stream decompressedStream = new GZipStream(langStream, CompressionMode.Decompress))
                        {
                            Debug.WriteLine("leyendo lenguaje " + language);
